...
class VPS {
    ...
    init(arsession: ARSession,
        ...
        self.failerConst = settings.maxFailsCount
        if gpsUsage {
            locationManager.attemptLocationAccess()
        }
    ...
    func sendRequest(meta: UploadVPSPhoto) {
        ...
        network.singleLocalize(photo: meta, locationIds: settings.locationIds) { (ph) in
    ...
    func getPosition(frame: ARFrame?, orient: Int) -> UploadVPSPhoto? {
        var intrinsics:(fx: Float, fy: Float, cx: Float, cy: Float, width: UInt, height: UInt) =
            (fx: 1592.2678,
             fy: 1592.2678,
             cx: 935.7558,
             cy: 538.8366,
        ...
        if let fr = frame {
            intrinsics = (fr.camera.intrinsics.columns.0.x,
                          ...
                          UInt(fr.camera.imageResolution.height),
                          UInt(fr.camera.imageResolution.width))
        }
        switch orient {
        case 1:
            var intr = intrinsics
        ...
        var uploadReq = UploadVPSPhoto(sessionID: self.sessionId,
                                       ...
                                       width: intrinsics.width / 2,
                                       height: intrinsics.height / 2,
                                       image: nil,
