...
class Network: NSObject {
    ...
    init(settings: Settings) {
        ...
        super.init()
        baseURL = "\(settings.url)vps/api/v3"
    }
    ...
    private func commonParsingResponse(_ response: (data: NSDictionary?, response: URLResponse?),
        ...
        if let json = response.data {
            ...
            if data == nil || headerError != nil {
                statusOk = false
                var errorMsg: String
                if let errData = unwrap(json, "detail")?[0], errData != json {
                    var errorField: String = ""
                    let errorType: String
                    if let errorFieldPath = errData["loc"] as? [String] {
                        for er in errorFieldPath {
                            errorField = "\(errorField)/\(er)"
                        }
                    }
                    errorMsg = parseString(errData, for: "msg")
                    errorType = parseString(errData, for: "type")
                    
                    errorMsg = "ErrorType: \(errorType). Msg: \(errorMsg)."
                    if errorField != "" {
                        errorMsg = "\(errorMsg) Error field: \(errorField)."
                    }
                }
                else {
                    errorMsg = parseString(json, for: "detail")
                    if errorMsg == "" {
                        errorMsg = "Could not parse server description error"
                    }
                }
