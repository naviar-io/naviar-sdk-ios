...
class VPSNMobileTests: XCTestCase {
    ...
    func testSetSettings() {
        var settings:Settings = Settings(locationIds: [""],
    ...
    func testInitServerVPS() {
        ...
        let settings:Settings = Settings(locationIds: [""],
                                         recognizeType: .server)
    ...
    func testInitPublicStruct() {
        ...
        let resp = ResponseVPSPhoto(status: true, vpsPose: ResponseVPSPhoto.VPSPose(posX: 1, posY: 1, posZ: 1, posRoll: 1, posPitch: 1, posYaw: 1))
        XCTAssertNotNil(gps)
